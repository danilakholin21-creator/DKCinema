...
public class MovieDetailActivity extends AppCompatActivity {
    ...
    private void setupReviewSection() {
        if (currentUserId != -1) {
            new Thread(() -> {
                Review existing = db.reviewDao().getUserReviewForMovie(currentUserId, movieId);
                runOnUiThread(() -> {
                    if (existing != null) {
                        ...
                        buttonSubmitReview.setOnClickListener(v -> {
                            ...
                            new Thread(() -> {
                                ...
                                runOnUiThread(() -> {
                                    Toast.makeText(MovieDetailActivity.this, "Спасибо за отзыв!", Toast.LENGTH_SHORT).show();
                                    layoutAddReview.setVisibility(View.GONE);
                                    refreshReviews();
                                });
                            }).start();
